...
from models.MSS_dataset import MSSMUSDBDataset, MSSMoisesDBDataset, MSSMUSMoisDBDataset, MSSCustomValidationDataset
...
class MSSSpecs(Dataset):
    ...
    def __init__(self, 
                 ...
                 enforce_full_mix_percentage=0.7, 
                 duration=5.0, 
                 dummy=False, 
                 normalize='noisy',
                 spec_transform=None,
        ...
        # Extract sr early so it can be used when creating datasets
        self.sr = stft_kwargs.get('sr', None) if stft_kwargs is not None else None
        
        # If base_dataset is provided, skip file path logic and use it directly
        if base_dataset is not None:
            self.dataset = base_dataset
        else:
            # Original dataset creation logic
            if data_dir is None:
                raise ValueError("data_dir must be provided if base_dataset is not specified")
            ...
            if dataset_str == "musdb":
                ...
                self.dataset = MSSMUSDBDataset(file_list, target_str, 
                                               ...
                                               valid_flag, rand_seed, sr=self.sr)
                ...
                # Feel free to add your own directory format
                raise NotImplementedError(f"Directory format {dataset_str} unknown!")
        
        # Common initialization for both base_dataset and path-based modes
        self.subset = subset
        self.dummy = dummy
        self.num_frames = num_frames
        self.normalize = normalize
        ...
        assert all(k in stft_kwargs.keys() for k in ["n_fft", "hop_length", "center", "window"]), "misconfigured STFT kwargs"
        # Store full kwargs but filter out 'sr' for actual STFT operations
        self.stft_kwargs = {k: v for k, v in stft_kwargs.items() if k != 'sr'}
        self.hop_length = self.stft_kwargs["hop_length"]
        assert self.stft_kwargs.get("center", None) == True, "'center' must be True for current implementation"
    ...
    def __getitem__(self, i):
        ...
        dataset_output = self.dataset[i]
        # Handle both 3-value return (regular datasets) and 4-value return (custom validation with file_id)
        if len(dataset_output) == 4:
            y, x, target_rms, file_id = dataset_output
            # Note: file_id is available when accessing dataset directly but not passed through
            # to maintain compatibility with batch processing in _step method
        else:
            y, x, target_rms = dataset_output
        ...
        if self.train_mono:
            ...
            if num_channels == 1:
                ...
                ch_id = np.random.randint(0, num_channels)
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        parser.add_argument("--base-dir", type=str, required=False, default="./", help="The base directory of the dataset. Should contain `train`, `valid` and `test` subdirectories, each of which contain `clean` and `noisy` subdirectories.")
        ...
        parser.add_argument("--dataset-str", type=str, choices=("musdb", "moisesdb", "musmoisdb"), default="musdb", help="Dataset to use. Default is musdb.")
        parser.add_argument("--target-str", type=str, choices=("vocals", "bass", "drums", "other", "all"), default="vocals", help="Target source to extract. 'vocals' by default.")
        parser.add_argument("--samples-per-track", type=int, default=1, help="How often each track is used in the dataset. Allows for oversampling of the dataset. 1 by default")
        parser.add_argument("--valid-split", type=float, default=0.1, help="Fraction of the training set to use for validation. 0.1 by default.")
        parser.add_argument("--use-musdb-test-as-valid", action="store_true", default=False, help="Use musdb test set as validation")
        parser.add_argument("--random-mix", action="store_true", default=False, help="Set for random mixing of sources.")
        parser.add_argument("--add-augmentation", action="store_true", default=False, help="Set for data augmentation.")
        parser.add_argument("--full-mix-percentage", type=float, default=0.7, help="Percentage of data with non-silent sources. 0.7 by default.")
        parser.add_argument("--rand-seed", default=13, type=int, help="Random seed that defines train/valid split.")
        parser.add_argument("--duration", type=float, default=5.0, help="Duration of the audio files. 5 seconds by default.")
        parser.add_argument("--batch-size", type=int, default=8, help="The batch size. 8 by default.")
        parser.add_argument("--train-mono", action="store_true", help="Use only the first channel of the audio files.")
        parser.add_argument("--n-fft", type=int, default=1534, help="Number of FFT bins. 1534 by default.")   # to assure 256 freq bins
        parser.add_argument("--hop-length", type=int, default=384, help="Window hop length. 384 by default.")
        ...
        parser.add_argument("--num-workers", type=int, default=8, help="Number of workers to use for DataLoaders. 8 by default.")
        parser.add_argument("--dummy", action="store_true", help="Use reduced dummy dataset for prototyping.")
        parser.add_argument("--spec-factor", type=float, default=0.065, help="Factor to multiply complex STFT coefficients by. 0.15 by default.")
        parser.add_argument("--spec-abs-exponent", type=float, default=0.667, help="Exponent e for the transformation abs(z)**e * exp(1j*angle(z)). 0.5 by default.")
        ...
        parser.add_argument("--transform-type", type=str, choices=("exponent", "log", "none"), default="exponent", help="Type of transformation to apply to the STFT. 'exponent' by default.")
        parser.add_argument("--custom-valid-dir", type=str, default=None, help="Custom validation directory with 'mixture' and 'target' subfolders containing paired WAV files.")
        return parser
    ...
    def __init__(
        ...
        gpu=True, normalize='noisy', transform_type="exponent", custom_valid_dir=None, **kwargs
    ):
        super().__init__()
        ...
        self.sr = sr
        self.custom_valid_dir = custom_valid_dir
        self.kwargs = kwargs

    def setup(self, stage=None):
        specs_kwargs = dict(
            stft_kwargs={**self.stft_kwargs, 'sr': self.sr}, num_frames=self.num_frames,
        ...
        if self.format=="MSS":
            if stage == 'fit' or stage == 'validate' or stage is None:
                ...
                # Use custom validation set if provided
                if self.custom_valid_dir is not None:
                    self.valid_set = MSSSpecs(base_dataset=MSSCustomValidationDataset(self.custom_valid_dir, sr=self.sr),
                                             normalize=self.normalize, train_mono=self.train_mono, **specs_kwargs)
                elif self.use_musdb_test_as_valid:
