...
if __name__ == "__main__":
    ...
    # Finetuning defaults to the provided BSRNN checkpoint unless explicitly overridden.
    if args.pretrained_ckpt is None:
        args.pretrained_ckpt = "./checkpoints/bsrnn_pretrained/bsrnn.ckpt"
        arg_groups["BSRNNSVSModel"].pretrained_ckpt = args.pretrained_ckpt

    # In the finetune entrypoint, default mode should be naive.
    if args.finetune_mode == "none":
        args.finetune_mode = "naive"
        arg_groups["BSRNNSVSModel"].finetune_mode = args.finetune_mode
    ...
    if logger is not None:
        ...
        # Track all four metrics as in finetune_sgmse.py
        callbacks.append(
            ModelCheckpoint(
                dirpath=join(args.log_dir, str(logger.version)),
                save_top_k=1,
