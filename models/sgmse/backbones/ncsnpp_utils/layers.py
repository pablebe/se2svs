...
class NINLinear(nn.Module):
  """NIN re-implemented with nn.Linear for PEFT/LoRA compatibility.

  Mathematically identical to NIN: applies a shared linear projection to every
  spatial position of a (B, C, H, W) feature map.  The only difference is that
  the weight lives inside a standard nn.Linear submodule ('nin_linear') so that
  PEFT can transparently wrap it with LoRA adapters.

  Checkpoint note: NIN stores W as (in_dim, num_units) and computes x @ W.
  nn.Linear stores weight as (num_units, in_dim) and computes x @ weight.T.
  Therefore: nin_linear.weight == NIN.W.T
  """
  def __init__(self, in_dim, num_units, init_scale=0.1):
    super().__init__()
    self.nin_linear = nn.Linear(in_dim, num_units, bias=True)
    with torch.no_grad():
      W = default_init(scale=init_scale)((in_dim, num_units))  # (in, out)
      self.nin_linear.weight.copy_(W.T)                         # (out, in)
      self.nin_linear.bias.zero_()

  def forward(self, x):
    x = x.permute(0, 2, 3, 1)   # (B, C, H, W) -> (B, H, W, C)
    y = self.nin_linear(x)       # (B, H, W, C')
