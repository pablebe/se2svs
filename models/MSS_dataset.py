...
class MSSMUSDBDataset(data.Dataset):
    def __init__(self, 
        ...
        # Target sample rate (for resampling if needed)
        self.target_sr = sr if sr is not None else self.fs
        self.need_resample = (self.fs != self.target_sr)
    ...
    def get_training_sample(self, 
        ...
        if self.need_resample:
            ...
            mixture_resampled = np.stack([librosa_resample(mixture_np[i], orig_sr=self.fs, target_sr=self.target_sr) for i in range(mixture_np.shape[0])], axis=0)
            target_resampled = np.stack([librosa_resample(target_np[i], orig_sr=self.fs, target_sr=self.target_sr) for i in range(target_np.shape[0])], axis=0)
            mixture = torch.Tensor(mixture_resampled)
            target = torch.Tensor(target_resampled)

        return mixture, target, target_rms
...
class MSSCustomValidationDataset(data.Dataset):
    """Dataset for custom validation set with paired mixture/target WAV files."""
    
    def __init__(self, base_dir, sr=None):
        """
        Args:
            base_dir: Directory containing 'mixture' and 'target' subdirectories
            sr: Target sample rate for resampling (if None, keep original)
        """
        super().__init__()
        self.base_dir = base_dir
        self.mixture_dir = os.path.join(base_dir, 'mixture')
        self.target_dir = os.path.join(base_dir, 'target')
        self.fs = sr
        # Get all mixture files
        self.mixture_files = sorted(glob(os.path.join(self.mixture_dir, 'mixture_fileid_*.wav')))
        

        temp_wav = self.mixture_files[0]
      
        self.fs_original = soundfile.info(temp_wav).samplerate

        # Verify paired target files exist
        self.file_ids = []
        for mix_path in self.mixture_files:
            file_id = os.path.basename(mix_path).replace('mixture_fileid_', '').replace('.wav', '')
            target_path = os.path.join(self.target_dir, f'target_fileid_{file_id}.wav')
            if os.path.exists(target_path):
                self.file_ids.append(file_id)
        
        print(f"Found {len(self.file_ids)} paired validation files in {base_dir}")
    ...
    def __getitem__(self, item):
        ...
        # Load mixture and target
        mix_path = os.path.join(self.mixture_dir, f'mixture_fileid_{file_id}.wav')
        target_path = os.path.join(self.target_dir, f'target_fileid_{file_id}.wav')
        
        mixture, fs_mix = soundfile.read(mix_path, dtype='float32')
        target, fs_target = soundfile.read(target_path, dtype='float32')
        self.fs_original = fs_mix  # Assuming mixture and target have the same original sample rate
        # Resample if needed
        if self.fs is not None and fs_mix != self.fs:
            if mixture.ndim == 1:
                mixture = librosa_resample(mixture, orig_sr=fs_mix, target_sr=self.fs)
            else:
                mixture = np.stack([librosa_resample(mixture[:, i], orig_sr=fs_mix, target_sr=self.fs) 
                                   for i in range(mixture.shape[1])], axis=1)
        
        if self.fs is not None and fs_target != self.fs:
            if target.ndim == 1:
                target = librosa_resample(target, orig_sr=fs_target, target_sr=self.fs)
            else:
                target = np.stack([librosa_resample(target[:, i], orig_sr=fs_target, target_sr=self.fs) 
                                  for i in range(target.shape[1])], axis=1)
        
        # Convert to torch tensors and ensure proper shape (channels, samples)
        if target.ndim == 1:
            target = torch.from_numpy(target).unsqueeze(0).float()  # (1, samples)
        else:
            target = torch.from_numpy(target.T).float()  # (channels, samples)
        
        if mixture.ndim == 1:
            mixture = torch.from_numpy(mixture).unsqueeze(0).float()  # (1, samples)
        else:
            mixture = torch.from_numpy(mixture.T).float()  # (channels, samples)
        
        # Compute RMS of target for normalization (consistent with other datasets)
        # Convert back to numpy with [time, channels] format for RMS computation
        target_rms = compute_rms(target.numpy().T, return_stereo=True)
        
        return mixture, target, target_rms, file_id
