...
from scipy.signal import lfilter # filter
...
def calculate_loudness(signal,fs, G = [1.0, 1.0, 1.0, 1.41, 1.41]):
    ...
    j_range = np.arange(0,(T-T_g)/(T_g*step)).astype(int) # number of windows
    ...
    l = [-.691 + 10.0*np.log10(np.sum([G_current[i]*z[i,j.astype(int)] for i in range(n_channels)]) + 1e-12) \
             for j in j_range]
